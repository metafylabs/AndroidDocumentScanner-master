package com.android.imagecropperlib.libraries;

import android.graphics.Bitmap;

import com.android.imagecropperlib.utils.Image;
import com.android.imagecropperlib.utils.Math;
...
public class NativeClass {
    ...
    public Bitmap getScannedBitmap(Bitmap bitmap, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
        ...
        Mat dstMat = perspective.transform(Image.bitmapToMat(bitmap), rectangle);
        return Image.matToBitmap(dstMat);
    }
    ...
    private static Comparator<MatOfPoint2f> AreaDescendingComparator = new Comparator<MatOfPoint2f>() {
        public int compare(MatOfPoint2f m1, MatOfPoint2f m2) {
            ...
            return (int) java.lang.Math.ceil(area2 - area1);
        }
    };
    ...
    public MatOfPoint2f getPoint(Bitmap bitmap) {
        ...
        Mat src = Image.bitmapToMat(bitmap);

        // Downscale image for better performance.
        double ratio = DOWNSCALE_IMAGE_SIZE / java.lang.Math.max(src.width(), src.height());
        ...
        MatOfPoint2f result = Math.scaleRectangle(largestRectangle, 1f / ratio);
        return result;
    }
        ...
        for (int c = 0; c < 3; c++) {
            ...
            for (int l = 0; l < THRESHOLD_LEVEL; l++) {
                ...
                for (MatOfPoint contour : contours) {
                    MatOfPoint2f contourFloat = Math.toMatOfPointFloat(contour);
    ...
    private boolean isRectangle(MatOfPoint2f polygon, int srcArea) {
        MatOfPoint polygonInt = Math.toMatOfPointInt(polygon);

        if (polygon.rows() != 4) {
            return false;
        }

        double area = java.lang.Math.abs(Imgproc.contourArea(polygon));
        ...
        for (int i = 2; i < 5; i++) {
            double cosine = java.lang.Math.abs(Math.angle(approxPoints[i % 4], approxPoints[i - 2], approxPoints[i - 1]));
            maxCosine = java.lang.Math.max(cosine, maxCosine);
        }
